...
exports.getReviews = async (req, res, next) => {
  try {
    ...
    const skipNum = (pageNum - 1) * parsedLimit;
...
exports.getAbandonedCarts = async (req, res, next) => {
  try {
    ...
    // Support sorting by value (only sorts current page since pagination happens in DB)
    ...
    res.status(200).json({
      ...
      pagination: {
        currentPage: pageNum,
        limit: parsedLimit,
        totalPages: Math.ceil(totalCarts / parsedLimit),
        totalCarts
      },
      data: formattedCarts
    });
